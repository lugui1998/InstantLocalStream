...
struct SharedState {
    ...
    /// Raw samples dropped by consumers from earlier backend generations.
    /// The active consumer's live counter is added when queried.
    raw_overflow_accumulated: AtomicU64,
    /// Live overflow count for the active raw consumer, mirrored by intake so
    /// diagnostics never need to contend on the raw-consumer mutex.
    raw_overflow_current: AtomicU64,
...
fn accumulate_raw_overflow(shared: &SharedState, consumer: &Option<RawConsumer>) {
    let final_overflow = consumer.as_ref().map_or_else(
        || shared.raw_overflow_current.load(Ordering::Relaxed),
        RawConsumer::overflow_count,
    );
    shared
        .raw_overflow_accumulated
        .fetch_add(final_overflow, Ordering::Relaxed);
    shared.raw_overflow_current.store(0, Ordering::Relaxed);
}

fn raw_overflow_increased(previous: &mut u64, current: u64) -> bool {
    if current < *previous {
        // A backend generation changed between observing raw_generation and
        // acquiring raw_consumer. Treat the lower counter as the new baseline.
        *previous = 0;
    }
...
impl Stream {
    ...
    pub fn open(config: StreamConfig, backend: Box<dyn CaptureBackend>) -> Result<Stream> {
        ...
        let shared = Arc::new(SharedState {
            ...
            raw_overflow_accumulated: AtomicU64::new(0),
            raw_overflow_current: AtomicU64::new(0),
    ...
    pub fn stop(&mut self) {
        ...
        // The producer is stopped and intake is joined, so this is the final
        // authoritative count for the active generation. Taking the consumer
        // also makes repeated stop calls unable to add the same total twice.
        {
            let mut consumer = self
                ...
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            accumulate_raw_overflow(&self.shared, &consumer);
            *consumer = None;
        }
    ...
    /// Number of interleaved native f32 samples discarded because the raw
    /// real-time capture ring was full, accumulated across backend recovery.
    pub fn dropped_raw_samples(&self) -> u64 {
        self.shared
            .raw_overflow_accumulated
            .load(Ordering::Relaxed)
            .saturating_add(self.shared.raw_overflow_current.load(Ordering::Relaxed))
    }
    ...
    fn open_backend_once(shared: &Arc<SharedState>) -> Result<()> {
        ...
        // 新しい consumer を共有へ載せ、世代を進める（旧 consumer は drop）。
        {
            ...
            let mut rc = shared
                ...
                .lock()
                .unwrap_or_else(|e| e.into_inner());
            accumulate_raw_overflow(shared, &rc);
            *rc = Some(consumer);
        }
                    ...
                    *be = new_backend;
                    // 新 consumer を共有へ載せ替え（旧 consumer は drop）。最後に行う。
                    {
                        ...
                        let mut rc = self
                            ...
                            .lock()
                            .unwrap_or_else(|e| e.into_inner());
                        accumulate_raw_overflow(&self.shared, &rc);
                        *rc = Some(consumer);
                    }
                }
                Err(e) => {
...
fn run_intake(
    ...
    let mut last_raw_overflow = 0_u64;
    ...
    loop {
        ...
        let mut push_err: Option<Error> = None;
        {
            ...
            if let Some(rc) = rc_guard.as_mut() {
                ...
                let current_overflow = rc.overflow_count();
                shared
                    .raw_overflow_current
                    .store(current_overflow, Ordering::Relaxed);
                if raw_overflow_increased(&mut last_raw_overflow, current_overflow) {
                    // DROP_OLDEST in the raw ring creates a real media-time
                    // discontinuity even if normalized output remains queued.
                    // Mark the next chunk at the exact overflow episode; the
                    // consumer can derive any full-packet RTP gap from PTS.
...
mod tests {
    ...
    #[test]
    fn raw_drop_diagnostics_include_previous_and_active_generations() {
        let stream = Stream::open(
            StreamConfig::default(),
            Box::new(MockBackend::new(48_000, 2, 440.0)),
        )
        .unwrap();

        // Simulate overflow after intake last mirrored the active count. A
        // backend replacement must read the consumer's final counter rather
        // than lose this last delta.
        let (mut producer, consumer) = raw_ring(2);
        assert_eq!(producer.push_slice(&[1.0, 2.0, 3.0, 4.0]), 2);
        ...
        *guard = Some(consumer);
        ...
        accumulate_raw_overflow(&stream.shared, &guard);
        assert_eq!(stream.dropped_raw_samples(), 2);

        // The next generation's mirrored live count is added without
        // changing the already accumulated total.
        stream
            .shared
            .raw_overflow_current
            .store(5, Ordering::Relaxed);
        assert_eq!(stream.dropped_raw_samples(), 7);
    }

    #[test]
    fn raw_overflow_episode_detection_handles_generation_counter_rollback() {
        let mut previous = 5;

        // Intake can briefly see the old consumer after observing the new
        // generation, then see the new consumer's zero-based counter.
        assert!(!raw_overflow_increased(&mut previous, 5));
        assert!(!raw_overflow_increased(&mut previous, 0));
        assert!(raw_overflow_increased(&mut previous, 1));
        assert!(!raw_overflow_increased(&mut previous, 1));
    }
}
