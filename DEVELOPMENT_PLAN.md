...
- Persisted UI preferences under an OS-owned temporary-directory namespace; only explicitly locked automatic or custom tokens are persisted.
...
Automatic tokens default to twelve ASCII letters or digits and can be configured from eight to 256 characters. The native UI also supports case-insensitive display/authentication, explicitly locked automatic tokens, and custom tokens with a security warning. The application also supports `--port N` as a convenience mode that uses the same numeric port for HTTP/TCP and the shared WebRTC UDP mux. All viewer sessions share one UDP port; the configured viewer cap is therefore independent of the port count.
